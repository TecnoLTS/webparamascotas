...
const isBillingApiRoute = (route) => route.path.startsWith('/api/{apiMode}/v1/')
...
const assertCapabilityIntegrity = (capabilities, backendRoutes) => {
...
const buildGeneratedJson = (capabilities, backendRoutes, billingApiRoutes, frontendPages, frontendHandlers, apiUses) => {
  const backendGatewayRoutes = backendRoutes.filter((route) => !isBillingApiRoute(route))
  const backendByCapability = groupRoutesByCapability(backendGatewayRoutes)
  const billingByCapability = groupRoutesByCapability(billingApiRoutes)

  const generatedCapabilities = capabilities.map((capability) => ({
    ...capability,
    backendRoutes: backendByCapability.get(capability.id) ?? [],
    billingApiRoutes: billingByCapability.get(capability.id) ?? [],
  }))
...
const buildGeneratedTs = (backendRoutes, billingApiRoutes) => {
  const allRoutes = backendRoutes.filter((route) => !isBillingApiRoute(route)).map((route) => ({
    id: route.id,
    method: route.method,
    path: route.path,
...
const main = () => {
  ...
  const billingApiRoutes = backendRoutes.filter(isBillingApiRoute)
  const frontendPages = loadFrontendPages()
  const frontendHandlers = loadFrontendHandlers()
  const apiUses = extractFrontendApiUses()

  assertCapabilityIntegrity(capabilities, backendRoutes)
  assertFrontendCoverage(capabilities, frontendPages, frontendHandlers, apiUses, backendRoutes)

  const generatedJson = `${JSON.stringify(buildGeneratedJson(capabilities, backendRoutes, billingApiRoutes, frontendPages, frontendHandlers, apiUses), null, 2)}\n`
  const generatedTs = buildGeneratedTs(backendRoutes, billingApiRoutes)
  ...
  console.log(`Capability registry ${action}: ${capabilities.length} capacidades, ${backendRoutes.length - billingApiRoutes.length} rutas backend tenantizadas, ${billingApiRoutes.length} rutas Billing publicas.`)
}
